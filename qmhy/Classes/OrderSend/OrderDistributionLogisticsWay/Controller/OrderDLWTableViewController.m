...
@interface OrderDLWTableViewController ()
...
@implementation OrderDLWTableViewController
...
- (void)loadNewData {
    ...
    [AFNetworkTool postJSONWithUrl:URL parameters:nil success:^(id responseObject) {
        ...
        _dataArray = [JSONModelOrderDLW objectArrayWithKeyValuesArray:_infoArray];
        NSLog(@"%@", _dataArray);
        // 刷新数据
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    // 1. 可重用标识符
    static NSString *ID = @"OrderDLWTableViewCell";
    // 2. tableView查询可重用Cell
    OrderDLWTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    // 3. 如果没有可重用cell
    if (cell == nil) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"OrderDLWTableViewCell" owner:nil options:nil] lastObject];
    }
